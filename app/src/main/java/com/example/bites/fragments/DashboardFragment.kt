package com.example.bites.fragments
...
import com.example.bites.adapters.DashboardFragmentAdapter
import com.example.bites.databinding.SortRadioButtonBinding
import com.example.bites.models.Restaurant
import com.example.bites.utils.ConnectionManager
...
class DashboardFragment(val contextParam: Context) : Fragment() {
    ...
    lateinit var radioButtonView: RadioGroup
...
//                    null
//                )
                radioButtonView = View.inflate(contextParam, R.layout.sort_radio_button, null) as RadioGroup

                var radioButtonHighToLow : RadioButton = radioButtonView.findViewById(R.id.radio_high_to_low)
                var radioButtonLowToHigh : RadioButton = radioButtonView.findViewById(R.id.radio_low_to_high)
                var radioButtonRating : RadioButton = radioButtonView.findViewById(R.id.radio_rating)
                androidx.appcompat.app.AlertDialog.Builder(activity as Context)
                    .setTitle("Sort By?")
                    .setView(radioButtonView)
                    .setPositiveButton("OK") { _, _ ->
                        if (radioButtonHighToLow.isChecked) {
                            Collections.sort(restaurantInfoList, costComparator)
                            restaurantInfoList.reverse()
                            dashboardAdapter.notifyDataSetChanged()
                        }
                        if (radioButtonLowToHigh.isChecked) {
                            Collections.sort(restaurantInfoList, costComparator)
                            dashboardAdapter.notifyDataSetChanged()
                        }
